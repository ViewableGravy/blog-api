...
export const publishDraftRoute = (mongoClient: MongoClient): ExpressRouteFunc => {
    return async (req: loginReqType, res: loginResType): Promise<any> => {
        ...
        const draftCollection = connection.db(dbName).collection(draftCollectionName);
        const publishCollection = connection.db(dbName).collection(publishCollectionName);
        ...
        // @ts-ignore
        res.status(201).send({postID: draft?.postID ?? creationResponse.upsertedId})
    };
};
